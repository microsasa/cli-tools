...
4. Make your decision:
   ...
   - If code quality is good but impact is HIGH: Add the label `aw-quality-gate-evaluated` to the PR, then add a comment explaining: what the high-impact areas are, why manual review is recommended, and what specifically a human reviewer should look at. Do NOT approve — auto-merge will remain blocked until a human approves.
   - If code quality is poor: Add the label `aw-quality-gate-evaluated` to the PR, then close the PR with a comment explaining the quality concerns (what's wrong and what needs to be fixed). The source issue stays open so the implementer can retry. Do NOT approve.
